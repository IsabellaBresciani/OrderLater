...
const ProductCard = ({ product }) => {
  ...
  const detailPath = `/shops/${product.shop}/products/${product._id}`;
  const editPath = `/shops/${product.shop}/products/${product._id}/edit`;
  const targetPath = isBusinessOwner ? editPath : detailPath;

  return (
    <div className="mb-4 text-center border-2 rounded-3 p-2">
      <Link to={targetPath} className="text-decoration-none">
        <div style={cardStyle}>
