...
class OrderController {
  ...
  getOrdersByUserId = async (request, response) => {
    ...
    const ordersWithActions = await this.orderService.getOrdersByUserId(
      userId,
      userIdFromToken
    );
  ...
  payOrder = async (request, response) => {
    try {
      ...
      const updatedOrder = await this.orderService.payOrder(id, userIdFromToken);

      return response.status(200).json({
        status: 'Success',
        message: `Order ${updatedOrder._id} payed successfully`,
        data: updatedOrder,
      });
    } catch (error) {
  ...
  cancelOrder = async (request, response) => {
    try {
      ...
      const updatedOrder = await this.orderService.cancelOrder(id, userIdFromToken);

      return response.status(200).json({
        status: 'Success',
        message: `Order ${updatedOrder._id} cancelled successfully`,
        data: updatedOrder,
      });
    } catch (error) {
